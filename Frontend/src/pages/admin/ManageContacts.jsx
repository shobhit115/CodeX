...
import { fetchAdminContacts, markMessageAsRead, deleteContactMessage } from "../../context/adminContactSlice";
...
export default function ManageContacts() {
  ...
  const { messages, loading, isLoaded } = useSelector((state) => state.adminContact);
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        {loading && !isLoaded ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-slate-500 text-sm">You're all caught up! No messages found.</p>
          </div>
        ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className={`text-sm truncate ${!msg.isRead ? "font-bold text-slate-900" : "font-medium text-slate-700"}`}>
                        {msg.name} <span className="font-normal text-slate-500 ml-1">({msg.email})</span>
                      </h4>
                    ...
                    <p className={`text-sm truncate ${!msg.isRead ? "font-semibold text-slate-800" : "text-slate-600"}`}>
                      {msg.subject}
                    </p>
                  </div>
